...
def isInRegion(pts, polygon):
	pt = Point(pts[0], pts[1])
	return polygon.contains(pt)

def findHypoPolygon(tar_dim, zangle, pos):
	# v1 = np.array([[pos[0]+tar_dim[0]/2], [pos[1]-tar_dim[1]/2], [1]])
	# v2 = np.array([[pos[0]+tar_dim[0]/2], [pos[1]+tar_dim[1]/2], [1]])
	# v3 = np.array([[pos[0]-tar_dim[0]/2], [pos[1]+tar_dim[1]/2], [1]])
	# v4 = np.array([[pos[0]-tar_dim[0]/2], [pos[1]-tar_dim[1]/2], [1]])
	v1 = np.array([[tar_dim[0]/2], [-tar_dim[1]/2], [1]])
	v2 = np.array([[tar_dim[0]/2], [tar_dim[1]/2], [1]])
	v3 = np.array([[-tar_dim[0]/2], [tar_dim[1]/2], [1]])
	v4 = np.array([[-tar_dim[0]/2], [-tar_dim[1]/2], [1]])
	...
	T = np.eye(3)
	T[0][0] = math.cos(zangle*math.pi/180)
	T[0][1] = -math.sin(zangle*math.pi/180)
	T[1][0] = math.sin(zangle*math.pi/180)
	T[1][1] = math.cos(zangle*math.pi/180)
	T[0][2] = pos[0]
	T[1][2] = pos[1]
	...
	new_v1 = [temp_v1[0][0], temp_v1[1][0]]
	new_v2 = [temp_v2[0][0], temp_v2[1][0]]
	new_v3 = [temp_v3[0][0], temp_v3[1][0]]
	new_v4 = [temp_v4[0][0], temp_v4[1][0]]

	xy = [new_v1, new_v2, new_v3, new_v4]
	print("xy:", xy)
	polygon = Polygon(xy)

	return polygon

def xyGenForAHypo(polygon, meshID):


	x_lowbound = p.getAABB(meshID)[0][0]
	x_upperbound = p.getAABB(meshID)[1][0]
	y_lowbound = p.getAABB(meshID)[0][1]
	y_upperbound = p.getAABB(meshID)[1][1]

	temp_x = random.uniform(x_lowbound, x_upperbound)
	temp_y = random.uniform(y_lowbound, y_upperbound)
	pt = Point(temp_x, temp_y)
	temp_pt = [temp_x, temp_y]
	while (not polygon.contains(pt)):
		temp_x = random.uniform(x_lowbound, x_upperbound)
		temp_y = random.uniform(y_lowbound, y_upperbound)
		pt = Point(temp_x, temp_y)
		temp_pt = [temp_x, temp_y]
